...
router.post('/', async (req, res) => {
    try {
        const manualId = "PD" + Date.now().toString(36) + Math.random().toString(36).substr(2);
        const rows = await loadCollection();
...
router.put('/', async (req, res) => {
    const rows = await loadCollection();
    await rows.updateOne({"manualId": req.body.manualId}, {$set: {name: req.body.name,price: req.body.price,quantity: req.body.quantity}})
    res.status(201).send();
});
...
async function loadCollection() {
    ...
    return client.db('bubble_bee').collection('products');
}
